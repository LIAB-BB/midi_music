      ...
      for (final measure in part.measures) {
        for (final voice in measure.voices) {
          for (final event in voice.events) {
            switch (event) {
              case ScoreNote note:
                if (note.isGrace && !includeGraceNotes) continue;
                ...
                if (!note.tieStart) {
                  final endPosition = _positionForAbsoluteBeat(
                    part,
                    note.position.absoluteBeat + note.durationBeats,
                  );
                  performanceEvents.add(
                    PerformanceEvent(
                      id: '${note.id}:off',
...
ScorePosition _positionForAbsoluteBeat(ScorePart part, double absoluteBeat) {
  for (var index = 0; index < part.measures.length; index += 1) {
    final measure = part.measures[index];
    final start = measure.startBeat;
    final end = start + measure.durationBeats;
    final isLastMeasure = index == part.measures.length - 1;
    if (absoluteBeat >= start && (absoluteBeat < end || isLastMeasure)) {
      return ScorePosition(
        measureNumber: measure.number,
        beat: absoluteBeat - start + 1,
        absoluteBeat: absoluteBeat,
      );
    }
  }
  return ScorePosition.absolute(absoluteBeat);
}
