...
void main() {
  ...
  test('解析同一 packet 中连续的多条 channel message', () {
    final events = parseMidiMessageBytes(
      Uint8List.fromList([
        0x90, 60, 96, // note on
        0x91, 64, 80, // note on, channel 1
        0xB1, 64, 127, // sustain down, channel 1
        0x80, 60, 0, // note off
      ]),
      timestamp: timestamp,
    );
    ...
    expect(events[2].sustain, isTrue);
  });
  ...
  test('忽略短消息、实时消息和暂未支持的消息', () {
    ...
    expect(
      parseMidiMessageBytes(
        Uint8List.fromList([0xE0, 0, 64, 0x90, 60, 1]),
        timestamp: timestamp,
      ).single.type,
      PerformanceInputEventType.noteOn,
    );
