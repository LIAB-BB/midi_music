...
> 状态：开发协议与服务骨架，不属于根 App 默认双入口，也不属于独立 K.478
> TestFlight 候选，更不是生产部署批准。当前服务缺少面向外部用户所需的
> 认证/授权、限流、TLS 终止、恶意文件防护、数据隔离、访问审计和正式删除
> 流程；恢复产品范围前须满足 [`product/release_scope.md`](product/release_scope.md)
> 的门槛。
...
受控开发验收阶段仅承诺支持清晰扫描或电子版钢琴五线谱。钢琴二重奏按多个
MusicXML part 转为多轨道；手写谱、吉他谱、管弦乐总谱和严重歪斜/低清
扫描件不纳入 MVP。

## App 配置

仅在受控环境中，通过 Dart define 配置服务端地址：
...
明确提示需要配置 PDF 识谱服务；这不构成试用版默认提供 PDF OMR 的承诺。
