...
适用对象：`apps/testflight_ios` + `packages/k478_practice`，iPhone-only、
iOS 13.0+。根目录默认 host（iOS 13.6+，K.478 交互五线谱 + USB
`PlayerPage` 双入口）及非默认 `home_page_legacy.dart` 的通用导入，都不构成
此清单的通过证据。

本候选仍使用 21 张静态 PNG 分谱和随包 Violin/Cello SF2，不包含根 App 的
MIDI→MusicXML→OSMD 交互谱面。若构建根 App，应执行
[`../release_checklist.md`](../release_checklist.md)。

自动化、真机、签名 IPA 是三条不同证据链；任一未完成都不能写成“已发布”。
建议将本次记录填入 [release dossier 模板](k478_release_dossier_template.md)。
...
- [ ] 候选 bundle 技术白名单只接受 K.478 MIDI、21 张 PNG、两个 SF2、
  `cupertino_icons` 图标字体与第三方通知；其中每项仍须在资产台账取得“允许”的
  分发许可，SF2/字体为“待批准”时即使通过白名单也不得分发。原始 PDF、TimGM
  和其他 MIDI 不得随包。
