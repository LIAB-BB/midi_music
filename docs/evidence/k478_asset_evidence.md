...
> 核验日期：2026-08-23。**这是 2026-08-23 至 2026-08-24 的历史技术证据
> 快照，不描述当前入口、当前构建或当前分发许可。** 当前双目标边界和分发结论
> 分别以 [`../product/release_scope.md`](../product/release_scope.md) 与
> [`asset_manifest.md`](asset_manifest.md) 为准。此记录已逐字节核验 K.478 MIDI
> 与源 PDF；钢琴分谱页面图已完成可复现渲染核验。弦乐 SoundFont 已完成许可
> 留档、成品哈希、结构和 Apple sampler 离线渲染验证，但输入 SF3 与 MuseScore
> 官方归档的字节级比对仍待完成。它不是对其他仓库资产的授权结论。
...
| 本地文件 | 对应原始文件 | SHA-256 | 2026-08-23 记录用途 |
| --- | --- | --- | --- |
| `assets/midi/mozart_k478_piano_quartet.mid` | `k478-score.mid` | `06f5457a2831d88dc4a7def8a8b35903c6a9be23ceeb30964c1147d9d280a025` | 根/候选 MIDI 的内容基准；当前分发状态另查台账 |
| `assets/scores/mozart_k478_piano_part.pdf` | `k478-piano-a4.pdf` | `d72a61d0102b797d9e89cb6c99741aa99d56f68af14ab78df15d5e244b0d12b4` | 对照和页面图来源；不得据此推断当前 Flutter bundle 已排除 PDF |
...
- **2026-08-23 历史边界：**当时仓库根目录只作为保留旧功能的开发 host，
  不属于当轮独立候选发布边界。当前 root 已是 K.478 交互五线谱 + USB
  `PlayerPage` 双入口的独立目标；当前边界必须查产品范围。独立候选的功能包
  仍只声明 K.478 MIDI、21 张钢琴分谱页面图、上述两个 SF2 与第三方授权通知；
  原始 PDF、其他 MIDI 和旧 TimGM6mb 不进入候选资产清单。
...
- **2026-08-24 历史工作区无签名构建**：当日在 `apps/testflight_ios` 执行
  `flutter build ios --release --no-codesign` 成功，生成的 `Runner.app` 为
  21.4 MB。`tool/verify_testflight_ios_bundle.sh build/ios/iphoneos/Runner.app`
  当时通过：实际 `UIDeviceFamily` 仅为 `[1]`，审计核验预期 framework、两个
  插件的 `PrivacyInfo`、K.478 MIDI、21 张 PNG、两个 SF2、第三方通知与
  `cupertino_icons` 字体；`FontManifest.json` 登记
  `packages/cupertino_icons/assets/CupertinoIcons.ttf`，host 顶层 Flutter assets
  和未知 package assets 均会被拒绝。此记录对应当时尚未提交的工作树；产物未
  签名、不是 Archive/IPA，也没有 Privacy Report，不能作为当前源码或分发证据，
  也不能替代 iPhone 真机、SF2 听音或签名分发核验。官方 CocoaPods CDN 当时在
  本机因证书链失败，故使用默认镜像；这不表示 CI 已运行或已通过。
